<?php session_start();
mysql_connect("chatmessage.db.7344823.hostedresource.com", "chatmessage", "y3llowW0mb@t") or die(mysql_error());
mysql_select_db("chatmessage") or die(msql_error());
...
if(isset($_GET['user']) && isset($_GET['message'])) {
	...
	$s = "SELECT username,message,posted FROM message WHERE (DAY(posted) = DAY(CURDATE()) AND MONTH(posted) = MONTH(CURDATE()) AND YEAR(posted) = YEAR(CURDATE())) ORDER BY posted DESC";
	$q = mysql_query($s) or die(mysql_error());
...
?>
	...
	<div style="color:<?php echo $user_bg; ?>"><?php echo "<strong>" . $r['username'] . " says:</strong> " . date('g:i:s a', strtotime($r['posted']));
?></div>
